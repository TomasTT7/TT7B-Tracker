...
/*
	Reads the Satellite Information message with up to 19 satellites or times out.
	
	UBX-NAV-SAT (8 + 12 * numSvs bytes)
*/
uint8_t ZOE_M8B_get_satellites(uint8_t * buffer, uint32_t timeout, uint8_t * length)
{
	static uint8_t requestUBX_NAV_SAT[8] = {0xB5, 0x62, 0x01, 0x35, 0x00, 0x00, 0x36, 0xA3};
	ZOE_M8B_send_message(requestUBX_NAV_SAT, 8);
	...
	*length = 0;
	...
	for(uint8_t i = 0; i < 255; i++)
	{
		...
		if(i == 2 && buffer[i] != 0x01) return 3;
		if(i == 3 && buffer[i] != 0x35) return 3;
		if(i == 11) msg_len = 8 + 8 + 12 * buffer[i];
		if(i >= msg_len - 1) break;
	}
	
	uint8_t checksum = ZOE_M8B_verify_checksum(buffer, msg_len);
...
/*
	...
	1	GPS and QZSS only
	2	GPS, QZSS and SBAS
	3	GPS, QZSS, GLONASS and GALILEO
	4	GPS, QZSS, BEIDOU and GALILEO
	5	GPS, QZSS, and GALILEO
	6	GALILEO only
	7	GLONASS only
	8	BEIDOU only
...
uint8_t ZOE_M8B_set_GNSS_system(uint8_t mode)
{
	switch(mode)
	{
		...
		case 3:;
			static uint8_t gnss_gali_glo[68] = {0xb5, 0x62, 0x06, 0x3e, 0x3c, 0x00, 0x00, 0x20, 0x20, 0x07, 0x00, 0x08, 0x10, 0x00, 0x01, 0x00,
												0x01, 0x01, 0x01, 0x01, 0x03, 0x00, 0x00, 0x00, 0x01, 0x01, 0x02, 0x04, 0x08, 0x00, 0x01, 0x00,
												0x01, 0x01, 0x03, 0x08, 0x10, 0x00, 0x00, 0x00, 0x01, 0x01, 0x04, 0x00, 0x08, 0x00, 0x00, 0x00,
												0x01, 0x03, 0x05, 0x00, 0x03, 0x00, 0x01, 0x00, 0x01, 0x05, 0x06, 0x08, 0x0e, 0x00, 0x01, 0x00,
												0x01, 0x01, 0x55, 0x27};
			ZOE_M8B_send_message(gnss_gali_glo, 68);
			break;
		
		case 4:;
			static uint8_t gnss_gali_bei[68] = {0xb5, 0x62, 0x06, 0x3e, 0x3c, 0x00, 0x00, 0x20, 0x20, 0x07, 0x00, 0x08, 0x10, 0x00, 0x01, 0x00,
												0x01, 0x01, 0x01, 0x01, 0x03, 0x00, 0x00, 0x00, 0x01, 0x01, 0x02, 0x04, 0x08, 0x00, 0x01, 0x00,
												0x01, 0x01, 0x03, 0x08, 0x10, 0x00, 0x01, 0x00, 0x01, 0x01, 0x04, 0x00, 0x08, 0x00, 0x00, 0x00,
												0x01, 0x03, 0x05, 0x00, 0x03, 0x00, 0x01, 0x00, 0x01, 0x05, 0x06, 0x08, 0x0e, 0x00, 0x00, 0x00,
												0x01, 0x01, 0x55, 0x3F};
			ZOE_M8B_send_message(gnss_gali_bei, 68);
			break;
			
		case 5:;
			static uint8_t gnss_galileo[68] = {0xb5, 0x62, 0x06, 0x3e, 0x3c, 0x00, 0x00, 0x20, 0x20, 0x07, 0x00, 0x08, 0x10, 0x00, 0x01, 0x00,
											   0x01, 0x01, 0x01, 0x01, 0x03, 0x00, 0x00, 0x00, 0x01, 0x01, 0x02, 0x04, 0x08, 0x00, 0x01, 0x00,
											   0x01, 0x01, 0x03, 0x08, 0x10, 0x00, 0x00, 0x00, 0x01, 0x01, 0x04, 0x00, 0x08, 0x00, 0x00, 0x00,
											   0x01, 0x03, 0x05, 0x00, 0x03, 0x00, 0x01, 0x00, 0x01, 0x05, 0x06, 0x08, 0x0e, 0x00, 0x00, 0x00,
											   0x01, 0x01, 0x54, 0x23};
			ZOE_M8B_send_message(gnss_galileo, 68);
			break;
		
		case 6:;
			static uint8_t gnss_only_gali[68] = {0xb5, 0x62, 0x06, 0x3e, 0x3c, 0x00, 0x00, 0x20, 0x20, 0x07, 0x00, 0x08, 0x10, 0x00, 0x00, 0x00,
												 0x01, 0x01, 0x01, 0x01, 0x03, 0x00, 0x00, 0x00, 0x01, 0x01, 0x02, 0x04, 0x08, 0x00, 0x01, 0x00,
												 0x01, 0x01, 0x03, 0x08, 0x10, 0x00, 0x00, 0x00, 0x01, 0x01, 0x04, 0x00, 0x08, 0x00, 0x00, 0x00,
												 0x01, 0x03, 0x05, 0x00, 0x03, 0x00, 0x00, 0x00, 0x01, 0x05, 0x06, 0x08, 0x0e, 0x00, 0x00, 0x00,
												 0x01, 0x01, 0x52, 0xE3};
			ZOE_M8B_send_message(gnss_only_gali, 68);
			break;
			
		case 7:;
			static uint8_t gnss_only_glon[68] = {0xb5, 0x62, 0x06, 0x3e, 0x3c, 0x00, 0x00, 0x20, 0x20, 0x07, 0x00, 0x08, 0x10, 0x00, 0x00, 0x00,
												 0x01, 0x01, 0x01, 0x01, 0x03, 0x00, 0x00, 0x00, 0x01, 0x01, 0x02, 0x04, 0x08, 0x00, 0x00, 0x00,
												 0x01, 0x01, 0x03, 0x08, 0x10, 0x00, 0x00, 0x00, 0x01, 0x01, 0x04, 0x00, 0x08, 0x00, 0x00, 0x00,
												 0x01, 0x03, 0x05, 0x00, 0x03, 0x00, 0x00, 0x00, 0x01, 0x05, 0x06, 0x08, 0x0e, 0x00, 0x01, 0x00,
												 0x01, 0x01, 0x52, 0xC3};
			ZOE_M8B_send_message(gnss_only_glon, 68);
			break;
		
		case 8:;
			static uint8_t gnss_only_bei[68] = {0xb5, 0x62, 0x06, 0x3e, 0x3c, 0x00, 0x00, 0x20, 0x20, 0x07, 0x00, 0x08, 0x10, 0x00, 0x00, 0x00,
												0x01, 0x01, 0x01, 0x01, 0x03, 0x00, 0x00, 0x00, 0x01, 0x01, 0x02, 0x04, 0x08, 0x00, 0x00, 0x00,
												0x01, 0x01, 0x03, 0x08, 0x10, 0x00, 0x01, 0x00, 0x01, 0x01, 0x04, 0x00, 0x08, 0x00, 0x00, 0x00,
												0x01, 0x03, 0x05, 0x00, 0x03, 0x00, 0x00, 0x00, 0x01, 0x05, 0x06, 0x08, 0x0e, 0x00, 0x00, 0x00,
												0x01, 0x01, 0x52, 0xDB};
			ZOE_M8B_send_message(gnss_only_bei, 68);
			break;
...
/*
	...
	Maximum navigation update rate (GPS)
		Super-E mode		4Hz
		Continuous mode		18Hz
	...
	3	10Hz Measurement frequency, 10Hz Navigation frequency
...
uint8_t ZOE_M8B_set_update_rate(uint8_t rate)
{
	switch(rate)
	{
		...
		case 3:;
			static uint8_t rate10Hz[14] = {0xB5, 0x62, 0x06, 0x08, 0x06, 0x00, 0x64, 0x00, 0x01, 0x00, 0x01, 0x00, 0x7A, 0x12};
			ZOE_M8B_send_message(rate10Hz, 14);
			break;
...
/*
	UBX-NAV-SAT in BUFFER. Variable message length LEN.
	
	SATS		number of satellites in UBX-NAV-SAT
	SIG1_10		number of satellites with Carrier to Noise ratio (CN0) between 1 and 10dBHz
	SIG11_20	number of satellites with Carrier to Noise ratio (CN0) between 11 and 20dBHz
	SIG21_30	number of satellites with Carrier to Noise ratio (CN0) between 21 and 30dBHz
	SIG31_40	number of satellites with Carrier to Noise ratio (CN0) between 31 and 40dBHz
	SIG41		number of satellites with Carrier to Noise ratio (CN0) above 41dBHz
*/
							  ...
							  uint8_t * sig21_30, uint8_t * sig31_40, uint8_t * sig41)
{
	...
	*sig1_10 = 0;
	...
	for(uint8_t i = 0; i < _sats; i++)
	{
		uint8_t cn0 = buffer[16 + i * 12];
		
		if(cn0 >= 41) *sig41 = *sig41 + 1;
		else if(cn0 >= 31) *sig31_40 = *sig31_40 + 1;
		else if(cn0 >= 21) *sig21_30 = *sig21_30 + 1;
		else if(cn0 >= 11) *sig11_20 = *sig11_20 + 1;
		else if(cn0 >= 1) *sig1_10 = *sig1_10 + 1;
	}
}
