...
int main(void)
{
	...
	//ack = ZOE_M8B_AssistNow_Autonomous(1);							// enable AssistNow Autonomous
...
/*	uint8_t ANAstatus = 0;
	...
	while(1)														// wait for AssistNow Autonomous
	{
		checksum = ZOE_M8B_get_ANA_status(GPS_buffer, 230000);
		
		if(checksum)
		{
			ZOE_M8B_parse_ANA_status(GPS_buffer, &ANAstatus);
			if(!ANAstatus) break;
		}
	}
...
*/		
		...
		/* GPS - Wake Up */
		PORT_switch_enable(1);										// POWER to GPS on - after measurements by SENSORS
		SERCOM_USART_enable(4000000, baud_val);						// operating baud rate (9600 or 115200)
		...
		if(tout)													// previous request didn't time out, communication works
		{
			...
			while(1)
			{
				...
				if(attempts > 390)									// limit acquisition to 39s (390 attempts at 10Hz) - ephemeris cycle 18-36s
				{
		...
		/* AssistNow Autonomous */
/*		uint32_t ANAtimeout = RTC_get_current_count();
		...
		while(1)													// wait for AssistNow Autonomous
		{
			...
			if((RTC_get_current_count() - ANAtimeout) > 2880) break;	// 90s timeout
...
*/		
		...
		n = APRS_packet(aprs_buffer, (uint8_t *)"OK7DMT", 2, lat, lon, (uint16_t)hMSL, MCU_temp, therm1_raw, therm2_raw, MS1_temp, MS2_temp,
